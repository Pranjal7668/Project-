...
function setupDashboardApp(token) {
  ...
  // Recalculate GST based on Freight amount and type selection
  const calculateGstValues = () => {
    ...
    if (gstType === 'IGST') {
      igst = Math.round(freight * 0.18 * 100) / 100;
    } else if (gstType === 'CGST_SGST') {
      cgst = Math.round(freight * 0.09 * 100) / 100;
      sgst = Math.round(freight * 0.09 * 100) / 100;
    }
  ...
  const updateGrandTotal = () => {
    ...
    const grandTotal = freight + custom + misc + gstTotal;
  ...
  const downloadExcelInvoice = async (shipment) => {
    try {
      ...
      worksheet.getCell('B1').font = { name: 'Outfit', size: 16, bold: true, color: { argb: 'FF1E3A8A' } };

      worksheet.mergeCells('B2:G2');
      worksheet.getCell('B2').value = 'International Courier & Cargo Services';
      worksheet.getCell('B2').font = { name: 'Inter', size: 10, italic: true, color: { argb: 'FF64748B' } };
      ...
      worksheet.getCell('B4').font = { name: 'Inter', size: 10, bold: true };
      ...
      worksheet.getCell('B5').font = { name: 'Inter', size: 9, color: { argb: 'FF475569' } };

      worksheet.mergeCells('B6:G6');
      worksheet.getCell('B6').value = 'Phone: +91 9068451228, 7248637203 | Email: smenterprisesagra5@gmail.com';
      worksheet.getCell('B6').font = { name: 'Inter', size: 9, color: { argb: 'FF475569' } };
      ...
      titleCell.font = { name: 'Outfit', size: 14, bold: true, color: { argb: 'FF1E3A8A' } };
      ...
      worksheet.getCell('A11').font = { name: 'Outfit', size: 10, bold: true, color: { argb: 'FF1E3A8A' } };
      ...
      worksheet.getCell('A16').font = { name: 'Outfit', size: 10, bold: true, color: { argb: 'FF1E3A8A' } };
      ...
      worksheet.getCell('E20').font = { name: 'Outfit', size: 10, bold: true, color: { argb: 'FF1E3A8A' } };
      ...
      worksheet.getCell('A25').font = { name: 'Outfit', size: 10, bold: true, color: { argb: 'FF1E3A8A' } };
      ...
      worksheet.getCell('A26').font = { bold: true, size: 9 };
      ...
      worksheet.getCell('C26').font = { bold: true, size: 9 };
      ...
      worksheet.getCell('E26').font = { bold: true, size: 9 };
      ...
      worksheet.getCell('A' + nextRowIdx).font = { bold: true, size: 10 };
      nextRowIdx++;
      ...
      worksheet.getCell(`E${startBankRow + 6}`).font = { name: 'Inter', size: 9, color: { argb: 'FF64748B' } };

      // Signatory section
      worksheet.getCell('F' + (nextRowIdx + 1)).value = 'SM Enterprises';
      worksheet.getCell('F' + (nextRowIdx + 1)).font = { bold: true, size: 10 };
      worksheet.getCell('F' + (nextRowIdx + 2)).value = 'Authorized Signatory';
      worksheet.getCell('F' + (nextRowIdx + 2)).font = { size: 9, color: { argb: 'FF64748B' } };
      ...
      worksheet.eachRow((row) => {
        row.font = row.font || { name: 'Inter', size: 10 };
      });
  ...
  const openInvoiceModal = (shipment) => {
    ...
    // Reset GST Type selection to None
  ...
  document.getElementById('inv-edit-custom').addEventListener('input', updateGrandTotal);
  document.getElementById('inv-edit-misc').addEventListener('input', updateGrandTotal);
